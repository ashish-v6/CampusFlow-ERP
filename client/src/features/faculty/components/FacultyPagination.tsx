...
import { FacultyPaginationMeta } from "../types/faculty.types";

interface FacultyPaginationProps {
  paginationDetails: FacultyPaginationMeta;
  currentPage: number;
  setCurrentPage: (page: number) => void;
}

/**
 * Pagination component for Faculty directory.
 * Preserves exact layout, styling, and windowed pagination math from StudentPagination and UserPagination.
 */
export default function FacultyPagination({
  paginationDetails,
  currentPage,
  setCurrentPage,
}: FacultyPaginationProps): React.JSX.Element {
  ...
  return (
    <div className="p-4 sm:px-5 sm:py-4 border-t border-border flex flex-col sm:flex-row items-center justify-between gap-4 text-sm bg-muted/10 rounded-b-2xl">
      ...
      <div className="text-muted-foreground text-xs sm:text-sm">
        ...
        <span className="font-medium text-foreground">{total}</span> faculty
      </div>
