...
import { AlertCircle, Building2, Hash, CheckCircle2 } from "lucide-react";
import { updateDepartment } from "../services/department.service";
import { Department, DepartmentStatus, DepartmentUpdateDto } from "../types/department.types";
import { validateUpdateDepartment } from "../validation/department.validation";
...
export default function UpdateDepartmentForm({
  ...
  onSuccess,
  onCancel,
}: UpdateDepartmentFormProps): React.JSX.Element {
  // Editable fields initialized from current department record
  const [formData, setFormData] = useState<DepartmentUpdateDto>({
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    const validation = validateUpdateDepartment(formData);
    ...
    try {
      ...
      if (err instanceof AxiosError && err.response?.data?.message) {
        ...
        setServerError("Failed to update department. Please check details and try again.");
      }
    } finally {
      setSubmitting(false);
    }
    ...
    <form onSubmit={handleSubmit} className="space-y-6" noValidate>
      ...
      <div className="p-4 rounded-xl bg-accent/30 border border-border flex flex-col sm:flex-row justify-between gap-2 text-xs">
        ...
        <div>
          ...
          <span className="font-mono text-muted-foreground">{department.id}</span>
        </div>
      </div>
      ...
      <div className="space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-[11px] text-muted-foreground">
            Note: Deactivating a department automatically deactivates its programs.
          </p>
        </div>
      </div>
