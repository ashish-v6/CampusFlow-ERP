...
/**
 * Modal dialog wrapping the CreateStudentForm.
...
export default function CreateStudentModal({
  ...
  onClose,
  onSuccess,
}: CreateStudentModalProps): React.JSX.Element | null {
  if (!isOpen) {
    return null;
  }
      ...
      <div className="bg-card border border-border shadow-2xl rounded-2xl w-full max-w-2xl overflow-hidden animate-in zoom-in-95 duration-200 my-auto">
        ...
        <div className="p-6 border-b border-border/80 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-foreground tracking-tight">Create Student</h2>
              <p className="text-xs text-muted-foreground mt-0.5">
                Link an active user to an academic program as an enrolled student.
              </p>
            </div>
          </div>
          <button
